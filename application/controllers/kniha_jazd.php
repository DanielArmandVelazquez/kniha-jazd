 ...
 * Time: 21:47
 */
class kniha_jazd extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->load->helper('form');
        $this->load->library('form_validation');
        $this->load->model('jazdy_models');
    }
    public function index(){
        ...
        $data['temperatures'] = $this->jazdy_models->getRows();
    ...
    public function view($id){
        ...
        if(!empty($id)){
            $data['kniha_jazd'] = $this->jazdy_models->getRows($id);
            $data['title'] = $data['kniha_jazd']['measurement_date'];
